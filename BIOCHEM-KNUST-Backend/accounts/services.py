    ...
    if serializer.is_valid(raise_exception=True):
        ...
        if send_via == "email":
            ...
            email_success, email_message = send_email_notification(
                ...
                subject=f"Password Reset Code - {settings.BRAND_SITE_LABEL}",
...
def request_email_verification_service(request, serializer_class):
    ...
    email_subject = f"{settings.BRAND_SITE_LABEL} - Phone Verification Code"
    email_message = f"""
Hi {user.first_name},

Your phone verification code is: <strong style="font-size: 24px; letter-spacing: 4px;">{code}</strong>

Use this code to verify your phone number on the {settings.BRAND_SITE_LABEL} app.
