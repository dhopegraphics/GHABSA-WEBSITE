...
class CalendarService:
    ...
    def _generate_uid(prefix: str, identifier: str) -> str:
        ...
        return f"{hashlib.md5(hash_input.encode()).hexdigest()}@{settings.UID_DOMAIN}"
    ...
    def generate_events_calendar(
        ...
        try:
            # Create calendar
            cal_name = f"{settings.BRAND_SITE_LABEL} Events"

            cal = cls._create_base_calendar(
                name=cal_name,
                description=f"{settings.BRAND_FULL_NAME} Events"
            )
